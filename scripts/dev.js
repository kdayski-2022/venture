async function main({
    ...
    // INITIALIZE
    const ventureInitializeTx = await venture.initialize(vSettings.limitAmounts, vSettings.commissions, vSettings.wallets, vSettings.settings)
    await ventureInitializeTx.wait()
    console.log(await venture.walletAddressForSuccessFundraising())

    // ADD ACCEPTABLE TOKEN
    const ventureAddAcceptableTokenTx1 = await venture.addAcceptableToken(usdcAddress)
    await ventureAddAcceptableTokenTx1.wait()
    const ventureAddAcceptableTokenTx2 = await venture.addAcceptableToken(xdaiAddress)
    await ventureAddAcceptableTokenTx2.wait()

    // ADD PRIVELEGED USERS
    const ventureAddPrivelegedUsersTx = await venture.addPrivelegedUsers([superUser])
    await ventureAddPrivelegedUsersTx.wait()

    // APPROVE SOME TOKENS TO VENTURE CONTRACT
    const erc20ApproveTx1 = await usdc.connect(superUserAccount).approve(ventureAddress, params.contributeAmount)
    await erc20ApproveTx1.wait()
    const erc20ApproveTx2 = await xdai.connect(simpleUserAccount).approve(ventureAddress, params.contributeAmount)
    await erc20ApproveTx2.wait()

    // CONTRIBUTE
    const ventureContributeTx1 = await venture.connect(superUserAccount).contribute(usdcAddress, params.contributeAmount)
    await ventureContributeTx1.wait()
    const ventureContributeTx2 = await venture.connect(simpleUserAccount).contribute(xdaiAddress, params.contributeAmount)
    await ventureContributeTx2.wait()

    //CHECKS
    console.log(`Balance VENTURE USDC tokens ${await usdc.balanceOf(ventureAddress)}`)
    ...
    console.log(`Balance SuccessUser USDC tokens ${await usdc.balanceOf(successUser)}`)
    console.log(`Balance SuccessUser XDAI tokens ${await xdai.balanceOf(successUser)}`)
    ...
    const projectTokenApproveTx = await projectToken.approve(ventureAddress, params.distributeProjectTokenAmount * 10)
    ...
    if (vSettings.settings[1] == 2) {
        ...
        console.log(`Set Project Token Price ${params.currentProjectTokenPrice / 100}$`)
        const setProjectTokenPriceTx = await venture.setProjectTokenPrice(params.currentProjectTokenPrice)
        await setProjectTokenPriceTx.wait()
    }

    // DESTRIBUTE TOKENS
    const distributeProjectTokenTx = await venture.distributeProjectToken(params.distributeProjectTokenAmount, 0)
    await distributeProjectTokenTx.wait()
    // second time
    console.log(`Success Fee Amount Tokens ${await venture.calculateSuccessFeeAmountTokenForType2(params.distributeProjectTokenAmount)}`)
    const distributeProjectTokenTx2 = await venture.distributeProjectToken(params.distributeProjectTokenAmount, 0)
    await distributeProjectTokenTx2.wait()
    console.log(`Balance superUser Project Tokens ${await projectToken.balanceOf(superUser)}`)
